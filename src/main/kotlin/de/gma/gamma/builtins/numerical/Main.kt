...
fun populateNumerical(scope: Scope) {
    ...
    bindWithDoc(scope, "-", SubtractFunction, "subtract [a] from [a]")
    bindWithDoc(scope, "*", TimesFunction, "multiplies two number values")
    bindWithDoc(scope, "×", TimesFunction, "multiplies two number values")
    bindWithDoc(scope, "/", DividedByFunction, "divides [a] by [a]")
    bindWithDoc(scope, "÷", DividedByFunction, "divides [a] by [a]")
    bindWithDoc(scope, "^", PowerToFunction, "raise [a] to the [a]th power")
