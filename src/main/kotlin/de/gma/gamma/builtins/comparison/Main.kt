...
fun populateComparison(scope: Scope) {
    ...
    bindWithDoc(scope, "!=", NotEqualFunction, "checks if <a> is not equal to <b>")
    bindWithDoc(scope, "≠", NotEqualFunction, "checks if <a> is not equal to <b>")
    ...
    bindWithDoc(scope, "≤", LessThanOrEqualFunction, "checks if the first value is less than or equal to the second")
    bindWithDoc(scope, "&", AndFunction, "returns true if <a> and <b> are both true")
    bindWithDoc(scope, "∧", AndFunction, "returns true if <a> and <b> are both true")
    bindWithDoc(scope, "|", OrFunction, "returns true if either <a> or <b> or both are true")
    bindWithDoc(scope, "∨", OrFunction, "returns true if either <a> or <b> or both are true")
